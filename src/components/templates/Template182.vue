
<template>
 <div><!-- -->
<table cellpadding="0" style="border-collapse: collapse; font-size: 13.6px;" width="500">
<tr>
<td style="margin: 0.1px; padding: 0px;">
<table cellpadding="0" style="border-collapse: collapse;">
<tr><!-- -->
<td style="margin: 0.1px; padding: 0px;" valign="top">
<table cellpadding="0" style="border-collapse: collapse;">
<tr>
<td style='margin: 0.1px; padding: 0px 0px 2px; font: 16.3px / 20.8px "Trebuchet MS", Helvetica, sans-serif; color: rgb(0, 0, 1); cursor: pointer;'>
<span style="font-weight: 600; color: rgb(0, 0, 0); cursor: pointer;">OLIVIA
                                                                                LANRON</span><!-- --><!-- -->
</td>
</tr>
<tr>
<td style='margin: 0.1px; padding: 0px 0px 8px; font: 600 13.6px / 17.3px "Trebuchet MS", Helvetica, sans-serif; color: rgb(0, 0, 1);'>
<span style="cursor: pointer;">Airbnb
                                                                                Super
                                                                                Host</span><!-- --><!-- -->
</td>
</tr>
<tr style="cursor: pointer;">
<td style='margin: 0.1px; padding: 1px 0px; font: 13.6px / 17.3px "Trebuchet MS", Helvetica, sans-serif; color: rgb(0, 0, 1);'>
<!-- --><span style='color: rgb(0, 0, 1); text-decoration: none; font-family: "Trebuchet MS", Helvetica, sans-serif;'>+1
                                                                                747 3847
                                                                                4442</span>, <!-- --><span style='color: rgb(0, 0, 1); text-decoration: none; font-family: "Trebuchet MS", Helvetica, sans-serif;'>olivia.airbnb@mysignature.io</span>
</td>
</tr><!-- --><!-- -->
<tr>
<td style='margin: 0.1px; padding: 10px 0px 0px; font: 13.6px / 17.3px "Trebuchet MS", Helvetica, sans-serif; color: rgb(0, 0, 1);'>
<table cellpadding="0" style="border-collapse: collapse;">
<tr>
<td style='margin: 0.1px; padding: 0px 4px 0px 0px; font: 13.6px / 17.3px "Trebuchet MS", Helvetica, sans-serif; color: rgb(0, 0, 1); cursor: pointer;'>
<span style="border-radius: 50%; width: 24px; height: 24px; display: block; text-align: center; background-color: rgb(253, 92, 99); font-size: 12px; font-family: initial; line-height: 1;"><svg style="position: relative; top: 4.8px; width: 14.4px;" viewbox="0 0 1024 1024" xmlns="http://www.w3.org/2000/svg">
<path d="M511.416 778.632c-57.629-72.292-91.516-135.633-102.812-189.895-11.21-43.733-6.777-78.728 12.404-105.028 20.332-30.263 50.596-44.969 90.365-44.969s69.991 14.706 90.323 45.268c19.010 26.002 23.785 61.039 12.191 105.028-12.404 55.327-46.248 118.668-102.769 189.938l0.298-0.342zM920.488 827.224c-7.885 53.068-44.074 97.1-93.775 118.583-95.949 41.772-190.961-24.851-272.289-115.215 134.482-168.369 159.332-299.441 101.618-384.222-33.887-48.593-82.395-72.249-144.627-72.249-125.445 0-194.413 106.136-167.304 229.323 15.772 66.666 57.587 142.453 124.252 227.191-41.772 46.206-81.372 79.070-116.366 99.402-27.11 14.663-53.068 23.742-77.919 25.916-114.108 17.007-203.577-93.69-162.956-207.882 5.627-14.706 16.837-41.772 36.018-83.587l1.066-2.259c62.36-135.377 138.105-289.254 225.145-459.924l2.259-5.627 24.723-47.527c19.181-35.038 27.067-50.723 57.587-70.033 14.706-8.908 32.779-13.384 53.068-13.384 40.664 0 72.334 23.742 85.889 42.881 6.734 10.187 14.706 23.742 24.808 40.621l23.785 46.376 3.41 6.777c86.955 170.585 162.785 324.163 224.889 459.882l1.108 1.066 22.719 52.002 13.555 32.565c10.358 26.087 12.532 52.045 9.079 79.155l0.26 0.168zM972.447 725.393c-7.928-24.851-21.483-54.176-38.363-89.214v-1.279c-80.433-170.713-155.155-324.163-226.083-462.056l-4.731-6.905c-50.554-103.664-86.742-165.939-191.856-165.939-103.92 0-148.079 72.206-193.219 166.11l-3.453 6.82c-71.099 137.849-145.778 291.512-225.955 462.141v2.259l-23.785 51.96c-8.951 21.483-13.512 32.736-14.706 36.104-57.544 158.224 61.039 297.182 204.6 297.182 1.151 0 5.627 0 11.253-1.151h15.857c74.594-9.079 151.447-56.478 229.408-141.345 77.919 84.738 154.857 132.266 229.323 141.345h15.857c5.627 1.151 10.145 1.151 11.253 1.151 143.561 0.128 262.144-138.957 204.6-297.182v-0.001z" fill="#ffffff">
</path>
</svg></span>
</td>
<td style='margin: 0.1px; padding: 0px 4px 0px 0px; font: 13.6px / 17.3px "Trebuchet MS", Helvetica, sans-serif; color: rgb(0, 0, 1); cursor: pointer;'>
<span style="border-radius: 50%; width: 24px; height: 24px; display: block; text-align: center; background-color: rgb(37, 211, 102); font-size: 12px; font-family: initial; line-height: 1;"><svg style="position: relative; top: 4.8px; width: 14.4px;" viewbox="0 0 1024 1024" xmlns="http://www.w3.org/2000/svg">
<path d="M654.906 563.048c11.234 0 118.954 56.172 123.579 64.102 1.322 3.305 1.322 7.27 1.322 9.912 0 16.522-5.287 35.025-11.234 50.225-15.2 37.008-76.659 60.799-114.327 60.799-31.722 0-97.146-27.755-125.561-40.973-94.502-42.955-153.317-116.309-210.151-200.238-25.112-37.008-47.582-82.607-46.919-128.206v-5.287c1.322-43.616 17.182-74.676 48.903-104.415 9.912-9.252 20.487-14.539 34.364-14.539 7.93 0 15.86 1.982 24.452 1.982 17.844 0 21.147 5.287 27.755 22.47 4.625 11.234 38.33 101.111 38.33 107.719 0 25.112-45.599 53.53-45.599 68.73 0 3.305 1.322 6.608 3.305 9.912 14.539 31.060 42.294 66.746 67.407 90.537 30.399 29.078 62.782 48.242 99.79 66.746 4.625 2.643 9.252 4.625 14.539 4.625 19.826 0 52.867-64.102 70.050-64.102l-0.003 0.001zM520.752 913.302c228.655 0 415.016-186.362 415.016-415.017s-186.361-415.016-415.016-415.016c-228.655 0-415.016 186.361-415.016 415.016 0 87.233 27.755 172.484 79.302 243.194l-52.207 153.979 159.927-50.885c67.407 44.278 147.371 68.73 227.995 68.73h-0.001zM520.752-0.001c274.915 0 498.282 223.37 498.282 498.284s-223.368 498.282-498.282 498.282c-83.928 0-167.196-21.147-241.21-62.119l-275.576 88.554 89.876-267.646c-46.919-77.319-71.371-166.535-71.371-257.073 0-274.915 223.368-498.282 498.282-498.282h-0.001z" fill="#ffffff">
</path>
</svg></span>
</td>
<td style='margin: 0.1px; padding: 0px 4px 0px 0px; font: 13.6px / 17.3px "Trebuchet MS", Helvetica, sans-serif; color: rgb(0, 0, 1); cursor: pointer;'>
<span style="border-radius: 50%; width: 24px; height: 24px; display: block; text-align: center; background-color: rgb(255, 0, 105); font-size: 12px; font-family: initial; line-height: 1;"><svg style="position: relative; top: 4.8px; width: 14.4px;" viewbox="0 0 1024 1024" xmlns="http://www.w3.org/2000/svg">
<path d="M682.653 512c0-93.991-76.659-170.654-170.654-170.654s-170.654 76.659-170.654 170.654c0 93.995 76.659 170.654 170.654 170.654s170.654-76.659 170.654-170.654zM774.646 512c0 145.323-117.325 262.645-262.645 262.645s-262.645-117.325-262.645-262.645c0-145.32 117.325-262.645 262.645-262.645s262.645 117.325 262.645 262.645zM846.641 238.688c0 33.997-27.331 61.328-61.328 61.328s-61.328-27.331-61.328-61.328c0-33.996 27.331-61.328 61.328-61.328s61.328 27.331 61.328 61.328v0zM512 92.033c-74.66 0-234.647-5.999-301.975 20.666-23.331 9.332-40.662 20.666-58.661 38.662s-29.33 35.33-38.662 58.661c-26.666 67.328-20.666 227.315-20.666 301.975s-5.999 234.647 20.666 301.975c9.332 23.331 20.666 40.662 38.662 58.661s35.33 29.33 58.661 38.662c67.328 26.666 227.315 20.666 301.975 20.666s234.647 5.999 301.975-20.666c23.331-9.332 40.662-20.666 58.661-38.662s29.33-35.33 38.662-58.661c26.666-67.328 20.666-227.315 20.666-301.975s5.999-234.647-20.666-301.975c-9.332-23.331-20.666-40.662-38.662-58.661s-35.33-29.33-58.661-38.662c-67.328-26.666-227.315-20.666-301.975-20.666zM1023.958 512c0 70.66 0.667 140.655-3.332 211.316-4 81.995-22.666 154.655-82.66 214.651s-132.656 78.66-214.651 82.66c-70.66 4-140.655 3.332-211.316 3.332s-140.655 0.667-211.316-3.332c-81.995-4-154.655-22.666-214.651-82.66s-78.66-132.656-82.66-214.651c-4-70.66-3.332-140.655-3.332-211.316s-0.667-140.655 3.332-211.316c4-81.995 22.666-154.655 82.66-214.651s132.656-78.66 214.651-82.66c70.66-4 140.655-3.332 211.316-3.332s140.655-0.667 211.316 3.332c81.995 4 154.655 22.666 214.651 82.66s78.66 132.656 82.66 214.651c4 70.66 3.332 140.655 3.332 211.316z" fill="#ffffff">
</path>
</svg></span>
</td>
</tr>
</table>
</td>
</tr>
</table>
</td>
</tr>
</table>
</td>
</tr>
<tr>
<td style='margin: 0.1px; padding: 15px 0px 0px; font: 13.6px / 17.3px "Trebuchet MS", Helvetica, sans-serif; color: rgb(0, 0, 1); cursor: pointer;'>
<span><img alt="created with MySignature.io" src="https://img.mysignature.io/b/2/3/0/230b3885-145e-557d-8c71-f3cb65ae984e.png?time=1685569257" style="display: block;" valign="top" width="409"/></span>
</td>
</tr>
</table>
<div>
<table cellpadding="0" style="border-collapse: collapse;" width="500">
<tr>
<td style="padding: 10px 10px 0px 0px; margin: 0.1px; cursor: pointer;">
<span><img alt="Check dates to stay in the best place to in New York &gt;&gt;&gt;" src="https://img.mysignature.io/a/v1/d/d/4/dd4eba98-274b-5dac-8a23-f9b7191b0dd7.png" style="max-width: 490px; display: block;" width="404"/></span>
</td>
</tr>
</table><!-- -->
<table border="0" cellpadding="0" cellspacing="0" width="500">
<tr>
<td style="margin: 0.1px; line-height: 1px; font-size: 1px; height: 1px;">
                                                         </td>
</tr>
</table>
</div><!-- -->
</div>
</template>

<script>
export default {
    name: 'Template182',
}
</script>
<style scoped></style>
    